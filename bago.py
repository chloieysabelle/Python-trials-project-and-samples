...
def appointment_window(full_name, age, sex):
    ...
    def schedule_appointment():
        ...
        IdNo = get_user_id()
        appointment_id = save_appointment_to_database(IdNo, appointment_info)

        if appointment_id is not None:
            print(f"Patient's Appointment ID: {appointment_id}")
            appointments[patient_name] = appointment_info  # Add to appointments dictionary
            show_appointment_summary(appointment_info, appointment_id)
...
def save_appointment_to_database(IdNo, appointment_info):
    global cursor
    # Your INSERT query
    insert_query = """
        INSERT INTO appointments (IdNo, patient_name, age, contact_number, sex, doctor, slot, appointment_date, reason, disease)
    ...
    def update_appointment():
        ...
        def save_updated_appointment(appointment_id):
            if appointment_id is None:
                messagebox.showerror("Error", "Invalid appointment ID. Please provide a valid ID.")
                return

            update_values = {key: entry.get() if hasattr(entry, 'get') else entry for key, entry in updated_info.items()}
            update_query = """
                UPDATE appointments 
                SET age=%s, contact_number=%s, sex=%s, reason=%s, disease=%s, doctor=%s, 
