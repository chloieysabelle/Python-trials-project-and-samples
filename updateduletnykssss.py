...
def create_login_box():
    ...
    login_admin_button = tk.Button(login_box, text="Login as Admin", command=lambda: (admin_log(), login_box.destroy()))
...
def admin_log():
    ...
    admin_email_label = tk.Label(login_box, text="Email Address:")
    admin_email_label.pack(anchor="w")
    admin_email_entry = tk.Entry(login_box)
    admin_email_entry.pack(fill="x", padx=10, pady=5)

    admin_password_label = tk.Label(login_box, text="Password:")
    admin_password_label.pack(anchor="w")
    admin_password_entry = tk.Entry(login_box, show="*")
    admin_password_entry.pack(fill="x", padx=10, pady=5)

    admin_login_button = tk.Button(login_box, text="Login", command=doctor_window)
