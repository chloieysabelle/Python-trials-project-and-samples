...
def delete_data(appointment_id):
    try:
        ...
        messagebox.showinfo("Deletion Successful", "Data deleted successfully!")

    except mysql.connector.Error as e:
        print("Error while deleting data:", str(e))
        messagebox.showerror("Deletion Error", "Failed to delete data. Please try again.")
...
def save_appointment_to_database(appointment_info):
    ...
    try:
        ...
        if total_appointments < MAX_APPOINTMENTS_PER_SLOT:
            ...
            IdNo = get_user_id() if get_user_id() is not None else 0  # Use a default value if user is not authenticated
            values = (
                    IdNo,
            ...
            query = "INSERT INTO appointments (IdNo, patient_name, age, contact_number, sex, reason, disease, doctor, appointment_time, appointment_date, amount) " \
...
def appointment_window(full_name, age, sex):
    ...
    def schedule_appointment():
        ...
        IdNo = get_user_id() if current_user and get_user_id() is not None else 0
...
def save_appointment_to_database(appointment_info):
    ...
    # Your INSERT query
    insert_query = """
        INSERT INTO appointments (IdNo, patient_name, age, contact_number, sex, doctor, appointment_time, appointment_date, reason, disease)
    ...
    try:
        ...
        messagebox.showinfo("Success", f"Appointment added successfully! Appointment ID: {last_inserted_id}")

        return last_inserted_id
    except mysql.connector.Error as e:
        print("Error while inserting into the database:", str(e))
        mydb.rollback()
        messagebox.showerror("Error", "Failed to add appointment. Please try again.")
...
def show_appointment_summary(appointment_info):
    ...
    def update_appointment():
        ...
        def save_updated_appointment(appointment_id):
            ...
            update_query = """
                UPDATE appointments 
                SET Name=%s, Age=%s, Contact_Number=%s, Sex=%s, Reason=%s, Disease=%s, Doctor=%s, 
                    Appointment_Time=%s, Appointment_Date=%s, Amount=%s
                WHERE AppointmentId=%s
            """
            update_values["Name"] = str(update_values["Name"])
            update_values["Age"] = int(update_values["Age"]) if update_values["Age"].isdigit() else 0  # Convert Age to int
            update_values["Contact_Number"] = str(update_values["Contact Number"])
            update_values["Amount"] = float(update_values["Amount"]) if update_values["Amount"].replace(".", "").isdigit() else 0.0
            update_values["Sex"] = str(update_values["Sex"])
            update_values["Reason"] = str(update_values["Reason"])
            update_values["Disease"] = str(update_values["Disease"])
            update_values["Doctor"] = str(update_values["Doctor"])
            update_values["Appointment_Time"] = str(update_values["Appointment Time"])
            update_values["Appointment_Date"] = str(update_values["Appointment Date"])

            update_values_tuple = (
                str(update_values["Name"]),
                ...
                str(update_values["Appointment_Time"]),
                str(update_values["Appointment_Date"]),
                float(update_values["Amount"]),
                appointment_id
            )
            ...
            try:
                ...
                mydb.rollback()
                messagebox.showerror("Error", f"Failed to update the appointment. Please try again. Error: {str(e)}")
...
background_image = Image.open("C:/Users/Chloie/Documents/project/finalpic.jpg")
